 ...
 * @since Jul 4, 2023
 */
@Component(immediate = true, name = "PTTimetableEntryIndexService", service = PTTimetableEntryIndexService.class, scope = ServiceScope.SINGLETON, reference = {
...
public class PTTimetableEntryIndexServiceImpl implements PTTimetableEntryIndexService {
	...
	private final static Logger LOGGER = Logger.getLogger(PTTimetableEntryIndexServiceImpl.class.getName());
	...
	public void activate() {
		factory.submit(() -> {
			...
			initializeIndex();
			return true;
		}).onSuccess(t -> LOGGER.info("Finished indexing PTTimetableEntry objects!"))
		.onFailure(t -> t.printStackTrace());	
	}
	
	private Promise<Void> initializeIndex() {
		LOGGER.info("Starting indexing PTTimetableEntry objects...");	
		
		resetIndex();

		EMFRepository repository = repositoryServiceObjects.getService();
		
		PushStream<EObject> pushStream = PTHelper.createPushStream(publicTransportPackage.getPTTimetableEntry(), repository);
		if(pushStream == null) return null;
		Promise<Void> resultPromise = pushStream
				.map(eo -> (PTTimetableEntry) eo)
				.forEach(p -> indexPTTimetableEntry(p, true));
		
		return resultPromise.onFailure(t -> repositoryServiceObjects.ungetService(repository))
				.thenAccept(result -> LOGGER.info("Finished indexing PTTimetableEntry object(s)!"))
				.thenAccept(result -> repositoryServiceObjects.ungetService(repository));
	}

	/* 
	 * (non-Javadoc)
	 * @see de.dim.trafficos.apis.publictransport.index.PTTimetableEntryIndexService#indexPTTimetableEntry(de.jena.udp.model.trafficos.publictransport.PTTimetableEntry, boolean)
	 */
	@Override
	public void indexPTTimetableEntry(PTTimetableEntry timetableEntry, boolean isFirstSave) {
		EObjectDocumentIndexObjectContext context = PTTimetableEntryIndexHelper.mapPTTimetableEntry(timetableEntry, isFirstSave ? IndexActionType.ADD : IndexActionType.MODIFY);			
		publicTransportIndex.handleContextSync(context);
	}

	/* 
	 * (non-Javadoc)
	 * @see de.dim.trafficos.apis.publictransport.index.PTTimetableEntryIndexService#deletePTTimetableEntry(de.jena.udp.model.trafficos.publictransport.PTTimetableEntry)
	 */
	@Override
	public void deletePTTimetableEntry(PTTimetableEntry timetableEntry) {
		EObjectDocumentIndexObjectContext context = PTTimetableEntryIndexHelper.mapPTTimetableEntry(timetableEntry, IndexActionType.REMOVE);			
		publicTransportIndex.handleContextSync(context);
	}

	/* 
	 * (non-Javadoc)
	 * @see de.dim.trafficos.apis.publictransport.index.PTTimetableEntryIndexService#resetIndex()
	 */
	@Override
	public void resetIndex() {
		try {
			...
			LOGGER.severe("Could not delete PTTimetableEntry index " + e);
		}	
	}
